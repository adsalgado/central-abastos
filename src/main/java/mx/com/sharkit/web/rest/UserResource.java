...
//    @PreAuthorize("hasRole(\"" + AuthoritiesConstants.ADMIN + "\")")
	...
	public ResponseEntity<UserDTO> getUser(@PathVariable String login) {
		...
		Optional<UserDTO> user = userService.getUserWithAuthoritiesByLogin(login).map(UserDTO::new);

		if (user.isPresent() && user.get().getTipoUsuarioId() == TipoUsuario.PROVEEDOR) {
			ProveedorDTO proveedor = proveedorService.findOneByusuarioId(user.get().getId()).orElse(null);
			if (proveedor != null) {
				user.get().setDireccion(proveedor.getDireccion());
			}
		}
