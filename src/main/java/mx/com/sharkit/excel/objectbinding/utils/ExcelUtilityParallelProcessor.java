...
public class ExcelUtilityParallelProcessor {
	...
	private final static Logger log = LoggerFactory.getLogger(ExcelUtilityParallelProcessor.class);
	...
	public <T extends Base> List<T> readExcelInParallel(ExcelFile excelFile) throws Exception {
		...
		try {
			List<Future<List<? extends Base>>> results = new ArrayList<Future<List<? extends Base>>>(
					excelFile.getNoOfthreadsToProcess());

			for (int i : excelFile.getSheetThreads().keySet()) {
				results.add(executorService
						.submit(new ExcelUtilityParallelReader(excelFile, excelFile.getSheetThreads().get(i), i == 1)));
			}
